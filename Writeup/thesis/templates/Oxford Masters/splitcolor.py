...
def pdfcolorsplit(
	...
	if merge:
		jobs = ((" ".join(ranges[0::2]), base_name + suffixes[iscolor[0]]), (" ".join(ranges[1::2]), base_name + suffixes[not iscolor[0]]))
	else:
		jobs = [(r, "%s_%03d%s" % (base_name, n + 1, suffixes[(n + iscolor[0]) % 2])) for (n, r) in enumerate(ranges)]
